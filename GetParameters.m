        ...
        for cpt_line = 1 : length(Text_from_file) - 1
            ...
            if ~strcmp(Text_from_file{cpt_line},'')
                ...
                if ~strcmp(Text_from_file{cpt_line}(1),'[')
                    ...
                    if length(strfind(ValueOfTheVariable,'.')) == 3 %check if this is an IP address
                        eval([NameOfTheVariable ' = ''' ValueOfTheVariable ''';']);
                    elseif all(ismember(ValueOfTheVariable, '0123456789+-.eEdD')) %then the ValueOfTheVariable is a number
